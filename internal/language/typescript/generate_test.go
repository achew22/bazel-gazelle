...
func TestGenerateRulesEmpty(t *testing.T) {
	...
	want := `typescript_library(name = "foo_typescript")`
	if got != want {
		t.Errorf("got:\n%s\nwant:\n%s", got, want)
	}
}
...
func TestGenerateFileInfo(t *testing.T) {
	lang := New()
	c := testConfig()
	dir := filepath.FromSlash("testdata/typescripts")
	_, gen := lang.GenerateRules(c, dir, "typescripts", nil, nil, []string{"foo.ts"}, nil, nil)
	r := gen[0]
	got := r.PrivateAttr(FileInfoKey).([]FileInfo)
	want := []FileInfo{{
		Path:        filepath.Join(dir, "foo.ts"),
		Name:        "foo.ts",
		PackageName: "bar.foo",
		Options:     []Option{{Key: "go_package", Value: "example.com/repo/typescripts"}},
		Imports: []string{
			"google/typescriptbuf/any.ts",
			"typescripts/sub/sub.ts",
		},
		HasServices: true,
	}}
